...
| `n0s1.mcp_tools.usage` | `usage_block(input_data, output_payload)` — token-savings estimation via cl100k_base; `input_data` accepts an `int` (raw character count from `SecretScanner.raw_chars_scanned`), a `dict` (report JSON) or `list` (findings array) whose baseline is the sum of all `mocked_secret` field lengths, or a `str` tokenised directly |
