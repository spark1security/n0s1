...
def analyze_report(
    ...
    *,
    ...
    until ai_analysis_status is "complete" or "failed".  Pass wait_minutes
    ...
    Args:
        ...
        wait_minutes: If set, poll the backend every 30 s until a terminal state or
                      this many minutes elapse.  Returns ai_analysis_status="timeout"
    ...
    Returns:
        AnalysisStatus with ai_analysis_status set to one of:
          ...
          "timeout"         — wait_minutes elapsed before analysis reached a terminal state
    """
    ...
    if wait_minutes is not None:
        ai_status = s.analyze_blocking(wait_minutes)
    else:
        ai_status = s.analyze()
    ...
    _STATUS_MESSAGES = {
        ...
        "timeout": f"Timed out after {wait_minutes}m — analysis still in progress. Call again to retry." if wait_minutes is not None else "Timed out waiting for analysis — still in progress. Call again to retry.",
    }
